...
function SubjectsListClient({
  filterParams,
}: {
  filterParams: TableSearchParams;
}) {
  const { data } = useQuery(subjectsQueries.list(filterParams));
        ...
        <div className="flex w-max flex-1 md:justify-end  flex-col md:flex-row items-center gap-4">
          ...
          <div className="flex gap-4  items-center self-end">
            ...
            <AllowedUserCompClient allowedRoles={["admin"]}>
              <FormDialog type="create" table="subject" relativeData={data.relativeData} />
            </AllowedUserCompClient>
          </div>
        </div>
      </div>

      <SubjectsTableWrapper data={data.data} userRole={data.userRole} relativeData={data.relativeData} />
...
export default SubjectsListClient;
