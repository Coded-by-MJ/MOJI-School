...
export default function SingleTeacherPageClient({ teacherId }: Props) {
  const router = useRouter();
  const { data, isLoading, isError } = useQuery(
    teachersQueries.getById(teacherId)
  );
  const { data: announcementData, isLoading: isLoadingAnnouncements } =
    useQuery(announcementsQueries.getRecent());

  // Handle redirect if teacher not found
  useEffect(() => {
